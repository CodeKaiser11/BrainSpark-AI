...
import { generateDynamicInsights } from '../services/analyticsService';

export default function AIInsights({ stats }) {
  const dynamicInsights = generateDynamicInsights(stats);
  ...
  const allInsights = [...dynamicInsights.map(d => ({
    type: d.type,
    text: d.text,
    icon: d.status === 'success' ? <FaCheckCircle className="text-green-400" /> : <FaExclamationTriangle className="text-orange-400" />,
    color: d.status === 'success' ? 'bg-green-400/10' : 'bg-orange-400/10'
  })), ...baseInsights];
